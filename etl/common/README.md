...
| File | Purpose |
|------|---------|
| [`aws.py`](aws.py) | AWS resource constants: `S3_BUCKET`, `BATCH_QUEUE`, `BATCH_JOB_DEFINITION`, `AWS_REGION`, `LAMBDA_NAME`, `ECR_*`. Override at runtime via env vars (`COEQWAL_S3_BUCKET`, `COEQWAL_BATCH_QUEUE`, `COEQWAL_BATCH_JOBDEF`). |
...
| `io.py` | Shared S3 JSON IO, SHA-256 streaming helpers (consolidated from `check_extraction_results.py` and `manual_ingest.py`) |
