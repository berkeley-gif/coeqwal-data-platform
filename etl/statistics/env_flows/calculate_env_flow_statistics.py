...
CHANNEL_ENTITY_CSV = (
    PROJECT_ROOT / "database/seed_tables/04_calsim_data/channel_entity.csv"
)
...
def load_channel_entities(csv_path: Optional[Path] = None) -> List[Dict[str, Any]]:
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            has_eflows = row.get("has_eflows", "false").strip().lower() in (
                "true",
                "t",
                "1",
            )
            ...
            reach_code = (
                network_arc_id[2:]
                if network_arc_id.startswith("C_")
                else network_arc_id
            )
...
def _extract_dv_columns(
    ...
    if missing:
        log.debug(
            f"DV: {len(missing)} target variables not found: {sorted(missing)[:10]}..."
        )
...
def _extract_sv_cfs_columns(
    ...
    if missing:
        log.debug(
            f"SV: {len(missing)} target variables not found: {sorted(missing)[:10]}..."
        )
    if taf_converted:
...
def load_dv_csv_from_s3(scenario_id: str, channels: List[Dict]) -> pd.DataFrame:
    ...
    for key in keys:
        try:
            ...
            resp = s3.get_object(Bucket=S3_BUCKET, Key=key)
            data_df = pd.read_csv(
                resp["Body"], header=None, skiprows=7, low_memory=False
            )
...
def calculate_seasonal_statistics(
    ...
    work.loc[dry_october_mask, "season_year"] = (
        work.loc[dry_october_mask, "WaterYear"] - 1
    )
    ...
    for season_code, season_info in CEFF_SEASONS.items():
        ...
        for p in EXCEEDANCE_PERCENTILES:
            row[f"flow_exc_p{p}"] = round(
                float(np.percentile(flow_by_year, 100 - p)), 3
            )
        ...
        if has_unimp:
            ...
            pct_u_by_year = (
                season_data.groupby("season_year")["pct_unimpaired"].mean().dropna()
            )
            if not pct_u_by_year.empty:
                ...
                for p in DELIVERY_PERCENTILES:
                    row[f"unimp_q{p}"] = round(
                        float(np.percentile(pct_u_by_year, p)), 3
                    )
                for p in EXCEEDANCE_PERCENTILES:
                    row[f"unimp_exc_p{p}"] = round(
                        float(np.percentile(pct_u_by_year, 100 - p)), 3
                    )
        ...
        if has_ef:
            pct_ff_by_year = (
                season_data.groupby("season_year")["pct_ff"].mean().dropna()
            )
            if not pct_ff_by_year.empty:
                ...
                row["target_met_pct"] = round(
                    float((arr >= 100.0).sum() / len(arr) * 100), 2
                )
...
def calculate_period_summary(
    ...
    if len(flow_valid) > 0 and "DaysInMonth" in df.columns:
        ...
        annual_flow_cfs = (
            ...
            .groupby("WaterYear")["flow"]
            .mean()
            .dropna()
        )
        annual_flow_taf = (
            pd.DataFrame({"WaterYear": water_years, "flow_taf": flow_taf})
            .groupby("WaterYear")["flow_taf"]
            .sum()
            .dropna()
        )
        if len(annual_flow_cfs) > 0:
            for p in EXCEEDANCE_PERCENTILES:
                result[f"flow_exc_p{p}_cfs"] = round(
                    float(np.percentile(annual_flow_cfs, 100 - p)), 3
                )
        if len(annual_flow_taf) > 0:
            for p in EXCEEDANCE_PERCENTILES:
                result[f"flow_exc_p{p}_taf"] = round(
                    float(np.percentile(annual_flow_taf, 100 - p)), 3
                )
...
def calculate_all_env_flow_statistics(
    ...
    merged = pd.merge(
        ...
        sv_df.drop(
            columns=drop_from_both + ["DaysInMonth", sv_df.columns[0]], errors="ignore"
        ),
