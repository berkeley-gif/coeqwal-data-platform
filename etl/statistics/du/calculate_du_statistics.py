...
def load_du_variable_mappings(
    conn, group_filter: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
    ...
    log.info(
        f"Loaded {len(results)} demand unit variable mappings"
...
def extract_delivery_data(
    df: pd.DataFrame, du_mapping: Dict[str, Any], delivery_arcs: Dict[str, List[str]]
) -> pd.Series:
    """
...
def extract_shortage_data(df: pd.DataFrame, du_mapping: Dict[str, Any]) -> pd.Series:
...
def calculate_shortage_monthly(
    df: pd.DataFrame, du_mapping: Dict[str, Any]
) -> List[Dict[str, Any]]:
...
def calculate_all_du_statistics(
    scenario_id: str, csv_path: Optional[str] = None, group_filter: Optional[str] = None
...
def save_to_database(
    ...
    try:
        ...
        for scenario_id in scenario_ids:
            ...
            cur.execute(
                "DELETE FROM du_shortage_monthly WHERE scenario_short_code = %s",
                (scenario_id,),
            )
            cur.execute(
...
def main():
    ...
    parser.add_argument(
        "--group", "-g", help="Filter to specific group (e.g., var_wba, var_gw_only)"
    )
    parser.add_argument(
        "--output-json", action="store_true", help="Output results as JSON"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Calculate but do not save to database"
    )
    parser.add_argument(
    ...
    for scenario_id in scenarios_to_process:
        try:
            delivery_monthly, shortage_monthly, period_summary, summary = (
                calculate_all_du_statistics(
                    scenario_id, csv_path=args.csv_path, group_filter=args.group
                )
            )
    ...
    save_to_database(
        all_delivery_monthly, all_shortage_monthly, all_period_summary, scenario_ids
    )
