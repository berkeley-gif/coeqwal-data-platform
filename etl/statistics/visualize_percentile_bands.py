...
def fetch_storage_data(api_url: str, scenario_id: str) -> dict:
    ...
    url = (
        f"{api_url}/api/statistics/scenarios/{scenario_id}/storage-monthly?group=major"
    )
...
def plot_percentile_band_chart(
    ...
    q0 = [
        monthly_data.get(str(m), monthly_data.get(m, {})).get("q0", 0)
        for m in range(1, 13)
    ]
    q10 = [
        monthly_data.get(str(m), monthly_data.get(m, {})).get("q10", 0)
        for m in range(1, 13)
    ]
    q30 = [
        monthly_data.get(str(m), monthly_data.get(m, {})).get("q30", 0)
        for m in range(1, 13)
    ]
    q50 = [
        monthly_data.get(str(m), monthly_data.get(m, {})).get("q50", 0)
        for m in range(1, 13)
    ]
    q70 = [
        monthly_data.get(str(m), monthly_data.get(m, {})).get("q70", 0)
        for m in range(1, 13)
    ]
    q90 = [
        monthly_data.get(str(m), monthly_data.get(m, {})).get("q90", 0)
        for m in range(1, 13)
    ]
    q100 = [
        monthly_data.get(str(m), monthly_data.get(m, {})).get("q100", 0)
        for m in range(1, 13)
    ]
    ...
    ax.fill_between(
        x, q10, q90, alpha=0.3, color="#3182bd", label="10th-90th percentile"
    )

    # Middle band: q30-q70 (medium blue)
    ax.fill_between(
        x, q30, q70, alpha=0.5, color="#3182bd", label="30th-70th percentile"
    )
    ...
    ax.plot(
        x, q0, color="#6baed6", linewidth=1, linestyle="--", alpha=0.7, label="Min/Max"
    )
    ...
    ax.set_title(
        f"{reservoir_name} ({reservoir_code}) - Monthly Storage Percentiles\n"
        f"Scenario: {scenario_id} | Capacity: {capacity_taf:,.0f} TAF",
        fontsize=14,
    )

    # Add horizontal line at 100% capacity
    ax.axhline(
        y=100, color="red", linewidth=1, linestyle=":", alpha=0.5, label="100% Capacity"
    )
...
def plot_all_reservoirs_grid(data: dict, scenario_id: str, output_dir: Path):
    ...
    for idx, res_code in enumerate(MAJOR_RESERVOIRS):
        ...
        # Extract percentiles
        q10 = [
            monthly.get(str(m), monthly.get(m, {})).get("q10", 0) for m in range(1, 13)
        ]
        q30 = [
            monthly.get(str(m), monthly.get(m, {})).get("q30", 0) for m in range(1, 13)
        ]
        q50 = [
            monthly.get(str(m), monthly.get(m, {})).get("q50", 0) for m in range(1, 13)
        ]
        q70 = [
            monthly.get(str(m), monthly.get(m, {})).get("q70", 0) for m in range(1, 13)
        ]
        q90 = [
            monthly.get(str(m), monthly.get(m, {})).get("q90", 0) for m in range(1, 13)
        ]
    ...
    fig.suptitle(
        f"Major Reservoirs - Monthly Storage Percentiles (Scenario: {scenario_id})",
        fontsize=14,
        fontweight="bold",
    )
...
def print_data_summary(data: dict):
    ...
    for code, res_data in reservoirs.items():
        ...
        print(
            f"    October percentiles: q10={oct_data.get('q10', 'N/A')}, "
...
def create_pdf_report(data: dict, scenario_id: str, output_dir: Path):
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for idx, res_code in enumerate(MAJOR_RESERVOIRS):
            ...
            q10 = [
                monthly.get(str(m), monthly.get(m, {})).get("q10", 0)
                for m in range(1, 13)
            ]
            q30 = [
                monthly.get(str(m), monthly.get(m, {})).get("q30", 0)
                for m in range(1, 13)
            ]
            q50 = [
                monthly.get(str(m), monthly.get(m, {})).get("q50", 0)
                for m in range(1, 13)
            ]
            q70 = [
                monthly.get(str(m), monthly.get(m, {})).get("q70", 0)
                for m in range(1, 13)
            ]
            q90 = [
                monthly.get(str(m), monthly.get(m, {})).get("q90", 0)
                for m in range(1, 13)
            ]
        ...
        fig.suptitle(
            f"Major Reservoirs - Monthly Storage Percentiles\nScenario: {scenario_id}",
            fontsize=14,
            fontweight="bold",
        )
        ...
        for res_code in MAJOR_RESERVOIRS:
            ...
            q0 = [
                monthly.get(str(m), monthly.get(m, {})).get("q0", 0)
                for m in range(1, 13)
            ]
            q10 = [
                monthly.get(str(m), monthly.get(m, {})).get("q10", 0)
            ...
            q100 = [
                monthly.get(str(m), monthly.get(m, {})).get("q100", 0)
                for m in range(1, 13)
            ]
            ...
            ax.set_ylabel("Storage (% of Capacity)", fontsize=12)
            ax.set_title(
                f"{name} ({res_code}) - Monthly Storage Percentiles\n"
                f"Scenario: {scenario_id} | Capacity: {capacity:,.0f} TAF",
                fontsize=14,
            )
            ax.axhline(
                y=100,
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize reservoir percentile band charts"
    )
    parser.add_argument(
        "--scenario", "-s", default="s0020", help="Scenario ID (default: s0020)"
    )
    parser.add_argument(
        "--api-url",
        default="https://api.coeqwal.org",
        help="API base URL (default: https://api.coeqwal.org)",
    )
    parser.add_argument(
        "--output-dir", "-o", default="./charts", help="Output directory"
    )
    parser.add_argument(
        "--summary-only", action="store_true", help="Just print data summary"
    )
