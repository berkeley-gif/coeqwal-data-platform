...
MI_CONTRACTORS_CSV = (
    PROJECT_ROOT / "database/seed_tables/04_calsim_data/mi_contractor.csv"
)
...
def load_mi_contractors(csv_path: Optional[Path] = None) -> Dict[str, Dict[str, Any]]:
    ...
    if not csv_path.exists():
        log.warning(
            f"mi_contractor.csv not found at {csv_path}, using built-in mappings"
        )
        ...
        for row in reader:
            ...
            if short_code:
                contractors[short_code] = {
                    ...
                    if row.get("contract_amount_taf")
                    else None,
                }
...
def load_calsim_csv_from_s3(
    scenario_id: str, variables: List[str]
...
def calculate_contractor_shortage_monthly(
    df: pd.DataFrame, contractor_code: str, shortage_vars: List[str]
) -> List[Dict[str, Any]]:
...
def _compute_perdv_demand_taf(
    ...
    for i, (d_var, s_var) in enumerate(zip(delivery_vars, shortage_vars)):
        ...
        s = (
            pd.to_numeric(df[s_var], errors="coerce").fillna(0)
...
def calculate_contractor_period_summary(
    ...
    df_copy = df.copy()
    df_copy["total_delivery"] = (
        df_copy[available_delivery].apply(pd.to_numeric, errors="coerce").sum(axis=1)
    )
    ...
    if perdv_vars:
        ...
        if avg_demand > 0:
            ...
            result["avg_pct_demand_met"] = round(
                safe_pct(
                    result["annual_delivery_avg_taf"],
                    avg_demand,
                    label=f"{contractor_code} pct_demand_met",
                    logger=log,
                ),
                2,
            )
        ...
        if MWD_TABLE_A_ANNUAL_TAF > 0:
            result["avg_pct_demand_met"] = round(
                safe_pct(
                    result["annual_delivery_avg_taf"],
                    MWD_TABLE_A_ANNUAL_TAF,
                    label=f"{contractor_code} pct_demand_met (Table A)",
                    logger=log,
                ),
                2,
            )
...
def calculate_all_mi_statistics(
    ...
    log.info(
        f"Pre-converted {cfs_converted} MI CFS→TAF columns; {taf_kept} already in TAF"
    )
    ...
    converted_cols = [
        c
        for c in df.columns
        if c in all_mi_vars and units_map.get(c, "").upper() == "CFS"
    ]
    if converted_cols:
        flagged = check_post_conversion_magnitude(df, converted_cols, logger=log)
        if flagged:
            log.warning(
                f"{flagged} MI columns exceed monthly TAF sanity limit after conversion"
            )
    ...
    for code, info in contractors.items():
        ...
        if has_shortage:
            shortage_rows = calculate_contractor_shortage_monthly(
                df, code, shortage_vars
            )
...
def main():
    ...
    for scenario_id in scenarios_to_process:
        try:
            delivery_monthly, shortage_monthly, period_summary = (
                calculate_all_mi_statistics(
                    scenario_id, csv_path=args.csv_path, use_local=args.use_local
                )
            )
    ...
    try:
        ...
        for scenario_id in scenario_ids:
            cur.execute(
                "DELETE FROM mi_delivery_monthly WHERE scenario_short_code = %s",
                (scenario_id,),
            )
            cur.execute(
                "DELETE FROM mi_shortage_monthly WHERE scenario_short_code = %s",
                (scenario_id,),
            )
            cur.execute(
                "DELETE FROM mi_contractor_period_summary WHERE scenario_short_code = %s",
                (scenario_id,),
            )
