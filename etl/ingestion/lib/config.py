"""Script-local constants for the Google Drive -> S3 ingestion pipeline.
...
backfill, etc). Anything that names a real AWS resource lives in
`etl.common.aws` instead.

Operator-tweakable: every constant in this file may need to change when the
...
# scenario's `sidecar.json` under `source.spreadsheet_url` so that a reader
# of the sidecar later (months from now, in S3, with no context) can find
# the upstream sheet a scenario row came from. Also surfaced in operator
# error messages when the working CSV is missing.
SPREADSHEET_URL = (
...
# Default directory for audit CSVs and per-run state. Gitignored via the
# umbrella `etl/**/audit_reports/` rule. Override with --output-dir. The
# audit_state JSON written here is the handoff record `audit.py` reads to
# know what gdrive_bulk_download.py saw on its most recent run (including
# scenarios that were skipped and never reached S3).
#
