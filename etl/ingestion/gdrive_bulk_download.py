...
of the working CSV (`etl/ingestion/scenario_listing/model_run_file_source_working.csv`):
  ...
  download  For each row, pull the ZIP, validate against the expected
            SV/DV basenames, hash everything, build the sidecar, and
            upload to `staging/scenario_data/<id>/`. Auto-renders
  ...
  promote   Copy each scenario's staged files to `ready/<id>/` in the
            safe order (sidecar -> trend csv -> ZIP last). The ZIP PUT
            under `ready/` is the Lambda trigger that releases extraction.
...
Outputs:
  ...
  s3://<bucket>/staging/scenario_data/<id>/<zip,trend.csv,sidecar.json>
  etl/ingestion/audit_reports/audit_report.csv  per-row CSV view
  etl/ingestion/audit_reports/audit_state.json  per-row JSON, consumed by audit.py
...
def main():
    ...
    _add_output_dir(sc, "scan_audit.csv")
    ...
    _add_output_dir(dl, "audit_report.csv and audit_state.json")
