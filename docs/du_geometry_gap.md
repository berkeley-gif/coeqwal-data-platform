...
short codes. The tier team should confirm the official names and
...
- Schema: [`database/sql_archive/04_scenario/56_add_du_geometry_columns.sql`](../database/sql_archive/04_scenario/56_add_du_geometry_columns.sql) adds `geom_wkt TEXT`, `srid INTEGER`, `geom geometry(MultiPolygon, 4326)`, and `idx_<table>_geom USING GIST` to each of the three `du_*_entity` tables.
