...
--
-- Renames:  scenario_id → short_code, short_code → run_name, short_title → name,
--           simple_description → short_description, description → long_description
-- Drops:    name (verbose title), subtitle, narrative, source_scenario_id, slr_id
-- Adds:     model_source_id (FK to model_source, all = 1)
-- Changes:  is_active INTEGER → BOOLEAN
--
...
-- ── 2. Drop FK constraints on columns being removed ─────────────────
...
-- ── 3. Drop indexes on columns being removed ────────────────────────
...
-- ── 5. Rename columns (order matters to avoid name conflicts) ───────
-- 5a. Move 'name' out of the way (it's the verbose title being dropped)
ALTER TABLE scenario RENAME COLUMN name TO _name_old;
-- 5b. Rename scenario_id → short_code (via temp to avoid conflict with existing short_code)
...
-- ── 7. Rename index on old short_code (now run_name) ────────────────
...
-- ── 10. Recreate scenario_full view with new column names ───────────
