...
-- ─── 1. env_flow_season ───────────────────────────────────────────────────────
-- Lookup table for the 5-season CEFF (California Environmental Flows Framework)
-- seasonal groupings used in the % functional flows metric.
-- Water year: October = month 1, September = month 12.
...
CREATE TABLE IF NOT EXISTS env_flow_season (
    ...
    description     TEXT,
    wy_months       INTEGER[]    NOT NULL,   -- water year month numbers in this season
    calendar_months TEXT         NOT NULL,   -- human-readable calendar month range
...
-- ─── 2. env_flow_channel_monthly ──────────────────────────────────────────────
-- Metric 1 — River flows as % of natural unimpaired flow, monthly aggregates.
-- One row per (network_arc_id, scenario_short_code, water_month).
-- Aggregated across all 80 simulated water years (or however many are in period of record).
-- NULL values where unimp_sv_variable is absent (e.g. MOK019, MOK028, canal arcs).

\echo 'Creating env_flow_channel_monthly...'

CREATE TABLE IF NOT EXISTS env_flow_channel_monthly (
    id                      SERIAL PRIMARY KEY,
    network_arc_id          VARCHAR(30)      NOT NULL,  -- References channel_entity.network_arc_id
    scenario_short_code     VARCHAR(20)      NOT NULL,
    water_month             SMALLINT         NOT NULL,  -- 1=Oct, 12=Sep

    -- Raw flow statistics (CFS) — stored to allow downstream unit conversion
    flow_avg_cfs            NUMERIC(12, 3),   -- mean of C_{reach} across years for this water_month
    flow_cv                 NUMERIC(8, 4),    -- coefficient of variation of monthly flow

    -- Unimpaired flow reference (CFS) — from UNIMP_{watershed} SV variable
    unimp_avg_cfs           NUMERIC(12, 3),   -- mean of UNIMP_{watershed} for this water_month

    -- % unimpaired statistics (unitless ratio × 100)
    -- NULL for reaches without a unimp_sv_variable (Mokelumne, canals)
    pct_unimpaired_avg      NUMERIC(8, 3),
    pct_unimpaired_cv       NUMERIC(8, 4),

    -- Percentile bands of pct_unimpaired across years for this water_month
    q0                      NUMERIC(8, 3),    -- minimum (driest year)
    q10                     NUMERIC(8, 3),
    q30                     NUMERIC(8, 3),
    q50                     NUMERIC(8, 3),    -- median
    q70                     NUMERIC(8, 3),
    q90                     NUMERIC(8, 3),
    q100                    NUMERIC(8, 3),    -- maximum (wettest year)

    -- Exceedance percentiles of pct_unimpaired
    exc_p5                  NUMERIC(8, 3),
    exc_p10                 NUMERIC(8, 3),
    exc_p25                 NUMERIC(8, 3),
    exc_p50                 NUMERIC(8, 3),
    ...
    sample_count            SMALLINT,          -- number of years contributing to aggregates
...
-- ─── 3. env_flow_channel_seasonal ─────────────────────────────────────────────
-- Metric 2 — River flows as % of functional flow (EFLOWS) targets, by CEFF season.
-- One row per (network_arc_id, scenario_short_code, season_id).
-- Only computed for reaches with has_eflows = true (~17 confirmed reaches).
-- NULL rows will not be written for reaches without EFLOWS variables.
...
CREATE TABLE IF NOT EXISTS env_flow_channel_seasonal (
    ...
    -- % of functional flow target, aggregated across years for this season
    -- pct_ff = C_{reach} / EFLOWS_{reach} × 100, averaged over season months in each year,
    -- then averaged (or distributed) across all years
    pct_ff_avg              NUMERIC(8, 3),    -- mean seasonal pct_ff across all years
    pct_ff_cv               NUMERIC(8, 4),    -- CV of per-year seasonal pct_ff

    -- Deviation from 100% target
    deviation_avg           NUMERIC(8, 3),    -- pct_ff_avg − 100.0 (negative = below target)

    -- Distribution of per-year seasonal pct_ff values
    q0                      NUMERIC(8, 3),
    q10                     NUMERIC(8, 3),
    q30                     NUMERIC(8, 3),
    q50                     NUMERIC(8, 3),    -- median year
    q70                     NUMERIC(8, 3),
    q90                     NUMERIC(8, 3),
    q100                    NUMERIC(8, 3),
    ...
    -- Fraction of years where seasonal pct_ff >= 100% (i.e. FF target met)
    target_met_pct          NUMERIC(6, 2),

    sample_count            SMALLINT,          -- number of years in the distribution
...
-- ─── 4. env_flow_channel_period_summary ───────────────────────────────────────
-- Metric 3 — Flow alteration index + full-period aggregate statistics.
-- One row per (network_arc_id, scenario_short_code).
-- pearson_r measures correlation between simulated flow and unimpaired natural flow
-- across the full 1,200-month period of record.
-- r ≈ +1: natural seasonal timing preserved; r ≈ 0: substantially altered.
...
CREATE TABLE IF NOT EXISTS env_flow_channel_period_summary (
    ...
    total_months                SMALLINT,        -- number of months in correlation

    -- Metric 3 — flow alteration index
    -- Pearson correlation between C_{reach} monthly series and UNIMP_{watershed} monthly series.
    -- NULL for reaches without a unimp_sv_variable (Mokelumne, canals).
    pearson_r                   NUMERIC(6, 4),
    p_value                     NUMERIC(8, 6),

    -- Full-period averages of Metric 1 (% unimpaired)
    avg_pct_unimpaired          NUMERIC(8, 3),   -- mean of all monthly pct_unimpaired values
    annual_cv_pct_unimpaired    NUMERIC(8, 4),   -- CV of annual-mean pct_unimpaired across years

    -- Full-period averages of Metric 2 (% functional flows)
    -- NULL for reaches without EFLOWS variable
    avg_pct_ff                  NUMERIC(8, 3),   -- mean of all monthly pct_ff values
    annual_cv_pct_ff            NUMERIC(8, 4),   -- CV of annual-mean pct_ff across years

    -- Fraction of all months where flow >= MIF threshold
    -- NULL for reaches without has_mif = true
    mif_met_pct                 NUMERIC(6, 2),   -- % of months where C_{reach} >= C_{reach}_MIF
