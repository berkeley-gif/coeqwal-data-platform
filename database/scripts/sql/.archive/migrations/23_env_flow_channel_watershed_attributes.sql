...
-- 1a. Add unimp_sv_variable column
...
-- 1b. Rename SAC_RIVER → SAC_UPPER and add SAC_LOWER
--     SAC_RIVER is split at Bend Bridge (rm 257): above = UNIMP_SHAS, at/below = UNIMP_SRBB.
UPDATE watershed
SET
...
-- 1c. Add missing watershed entries
INSERT INTO watershed (short_code, name, description, hydrologic_region_short_code, unimp_sv_variable, is_active)
VALUES
    ('UPPER_MERCED',
...
-- 1d. Populate unimp_sv_variable for all existing watersheds
...
-- BEAR_RIVER and UPPER_MOKELUMNE: no UNIMP variable in CalSim SV — leave NULL

\echo '  watershed: updated SAC_RIVER → SAC_UPPER, added SAC_LOWER + 3 new watersheds, unimp_sv_variable populated'
...
-- Populate new columns for all 60 DV channels
-- Format: (network_arc_id, watershed_short_code, unimp_sv_variable, has_mif, has_eflows, channel_class)
...
-- Insert MIF variables.  channel_entity_id is resolved via subquery on network_arc_id.
-- All MIF variables: is_regulatory = true, unit = CFS (unit_id=2), monthly (temporal_scale_id=3).
...
-- Add C_ISF001_OMR027 flow variable (missing from channel_variable)
INSERT INTO channel_variable
