-- LOAD TIER TABLES FROM S3 (FIXED)
-- Creates and populates tier_definition and tier_result tables
-- Includes audit metadata, versioning, and indexes

\echo ''
\echo '🎯 LOADING TIER TABLES FROM S3 (FIXED VERSION)'
\echo '=============================================='

\echo ''
\echo '📋 Checking version table structure...'
\d version

\echo ''
\echo '📋 Getting tier version ID...'
SELECT id as tier_version_id, version_family_id, version_number 
FROM version v
...
CREATE TABLE IF NOT EXISTS tier_definition (
    ...
    tier_version_id INTEGER NOT NULL DEFAULT 9,
...
\echo '✅ tier_definition table created'
...
\echo '✅ tier_result table created'
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'version') THEN
        ...
        RAISE NOTICE '✅ Version foreign keys added';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'developer') THEN
        ...
        RAISE NOTICE '✅ Developer foreign keys added';
    END IF;
    ...
    RAISE NOTICE '✅ Tier definition foreign key added';
END $$;
