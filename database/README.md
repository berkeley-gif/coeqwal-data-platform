...
**Note:** The tables and columns below are planned to support the Community Water Systems dataset. **None of it exists in the database yet:** The source spreadsheets are staged in [`data/reference/cws/`](../data/reference/cws/) (the spring-2026 delivery: `Master list of systems served for sw units updated april 13.xlsx`, `Updated HHS allocations May 6 2026.xlsx`, `Updated Master crosswalk SW DUs M&I May7 2026.xlsx`, `Systems_served_by_DU_systemname_updated.xlsx`, plus `Final_M&Idemandunits_withlatlongs.xlsx`). See [`data/reference/cws/README.md`](../data/reference/cws/README.md). No seed CSVs have been converted into `seed_tables/` yet. Check with the CWS team to make sure this is the latest before proceeding.
...
Today the database only has the CalSim DUs (classified as "urban" demand units). The planned tables below add the real-world water-system layer and relate it to the model's DUs. They do not add new wide columns to `du_urban_entity` (see the attribute-placement note after the table):
...
- **`gw` / `sw` boolean migration:** (see [`SCHEMA_BACKLOG.md` § 6](SCHEMA_BACKLOG.md#6-schema-pattern-inconsistencies) and [`gw_sw_reconciliation.md` Step 7](topic_docs/cws/gw_sw_reconciliation.md#step-7-boolean-type-migration-independent-of-value-reconciliation)) must land before those columns can serve as the SW/GW flags.
...
- **Publish a consolidated `coeqwal-locations-points` tileset:** Cross-reference for the "Hardcoded point coordinates" bullet above. One NDGeoJSON feed per outcome, joined into a single tileset whose features carry `outcome_code` (one of `AG_REV`, `CWS_DEL`, `ENV_FLOWS`, etc.) and `short_code` (the per-feature identifier the API uses). Register as a new source in [apps/main/app/features/map/config/tilesetSources.ts](../../coeqwal-website/apps/main/app/features/map/config/tilesetSources.ts), wire it into the affected outcome layers, then delete the coordinate tables from `outcomeLocations.ts`. The animation in `useTierAnimationData.ts` currently joins tier results to `AG_REV_COORDINATES` in TS.

- **Check that every demand unit the API serves has a matching map tile:** The `coeqwal-demand-units` tileset is already keyed by `DU_ID` (see `tilesetSources.ts`), so there is no re-keying to do. What is missing is a check that the tiles and the API agree. Write a small script in `scripts/mapbox_recipes/` that lists the DU IDs the API returns, lists the `DU_ID` values in the tileset features, and reports any demand unit that has no tile (and any tile with no demand unit). The demand units already known to be missing geometry are listed in [`topic_docs/geometry.md`](topic_docs/geometry.md). Changes to CWS locations will likely increase the missing geometry.
