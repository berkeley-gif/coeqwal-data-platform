...
class ScenarioFiles(BaseModel):
    ...
    output_csv: Optional[FileInfo] = Field(
        None, description="CalSim output time series CSV"
    )
            ...
            if "CommonPrefixes" in page:
                for prefix in page["CommonPrefixes"]:
                    ...
                    if prefix_path.startswith("scenario/") and prefix_path.endswith(
                        ...
                        scenario_id = prefix_path[
                            9:-1
                        ]  # Remove "scenario/" and trailing "/"
...
@router.get(
    "/scenario", response_model=ScenariosResponse, summary="List downloadable scenarios"
)
    ...
    try:
        ...
        for scenario_id in scenario_ids:
            ...
            if files.zip or files.output_csv or files.sv_csv:
                scenario = Scenario(scenario_id=scenario_id, files=files)
...
async def get_download_url(
    scenario: str = Query(
        ..., description="Scenario ID (e.g., 's0020')", example="s0020"
    ),
    type: str = Query(
        ..., description="File type: 'zip', 'output', or 'sv'", example="zip"
    ),
):
    """
